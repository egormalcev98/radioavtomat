...
class CreateOutgoingDocumentsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('outgoing_documents', function (Blueprint $table) {
            ...
            $table->string('document_type_id')->comment('вид документа');// вид документа
            ...
            $table->string('pages')->comment(' число страниц'); // число страниц
            $table->integer('incoming_letter_number')->comment('ответ на входящее письмо номер');; // Ответ на входящее письмо номер
            $table->timestamps();
            $table->softDeletes();
        });
    }
